...
class css_utils:
    ...
    def load_colors_variables():
        ...
        style_path = os.path.join(os.path.dirname(
            __file__), '..', 'assets', 'css', 'style.qss')
        with open(style_path, 'r') as file:
            lines = file.readlines()
            # dont get variables because it dont work in pyqt!!!
            content = ''.join(lines[13:])
        for color in tab:
            if os.environ.get(color):
                content = content.replace(
                    f'var(--{color})', os.environ.get(color))
            else:
                content = content.replace(
                    f'var(--{color})', app_custom_colors[color])
    ...
    def load_stylesheet(app, theme, extra, style_path='style.qss'):
        ...
        style_path = os.path.join(os.path.dirname(
            __file__), '..', 'assets', 'css', style_path)
...
class helpers:
    @staticmethod
    def get_color(color: QtMaterialEnv):
        return os.environ.get(color.value)
